...
VCvarB clip_shadow("clip_shadow", true, "Use clipper to drop unnecessary shadow surfaces?", CVAR_PreInit|CVAR_NoShadow);
VCvarB clip_advlight_regions("clip_advlight_regions", false, "Clip (1D) light regions?", CVAR_PreInit|CVAR_NoShadow);
...
static VVA_OKUNUSED inline void ClipSegToLight (TVec &v1, TVec &v2, const TVec &pos, const float radius) {
  ...
  const float d1 = pos.dot(r1.cross(r2).normalise());
  const float d2 = pos.dot(r2.cross(r1).normalise());
