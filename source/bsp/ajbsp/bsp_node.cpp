...
# if DEBUG_SPLIT
	if (old_seg->linedef)
		ajbsp_DebugPrintf("Splitting Linedef %d (%p) at (%1.1f,%1.1f)\n",
				old_seg->linedef->index, old_seg, x, y);
	else
		ajbsp_DebugPrintf("Splitting Miniseg %p at (%1.1f,%1.1f)\n", old_seg, x, y);
# endif
...
# if DEBUG_SPLIT
	ajbsp_DebugPrintf("Splitting Vertex is %04X at (%1.1f,%1.1f)\n",
...
# endif
	...
	{
#   if DEBUG_SPLIT
		ajbsp_DebugPrintf("Splitting Partner %p\n", old_seg->partner);
#   endif
						 ...
						 int best_cost)
{
	...
	{
#   if DEBUG_PICKNODE
		ajbsp_DebugPrintf("Eval : No real segs on %s%sside\n",
				info.real_left  ? "" : "left ",
...
# if DEBUG_PICKNODE
	ajbsp_DebugPrintf("Eval %p: splits=%d iffy=%d near=%d left=%d+%d right=%d+%d "
...
# if DEBUG_PICKNODE
	ajbsp_DebugPrintf("FindFastSeg: best_H=%p (cost %d) | best_V=%p (cost %d)\n",
...
# if DEBUG_PICKNODE
	ajbsp_DebugPrintf("PickNode: BEGUN (depth %d)\n", depth);
# endif
	...
	{
#   if DEBUG_PICKNODE
		ajbsp_DebugPrintf("PickNode: Looking for Fast node...\n");
#   endif
		...
		{
#     if DEBUG_PICKNODE
			ajbsp_DebugPrintf("PickNode: Using Fast node (%1.1f,%1.1f) -> (%1.1f,%1.1f)\n",
...
# if DEBUG_PICKNODE
	if (! best)
	{
		ajbsp_DebugPrintf("PickNode: NO BEST FOUND !\n");
	}
	else
	{
		ajbsp_DebugPrintf("PickNode: Best has score %d.%02d  (%1.1f,%1.1f) -> (%1.1f,%1.1f)\n",
	...
	for (num=0 ; num < 2 ; num++)
	{
		...
		if (A)
		{
			...
			if (A->real_num + A->mini_num > 0)
				ajbsp_BugError("SeparateSegs: child %d not empty!\n", num);
...
# if DEBUG_CUTLIST
	ajbsp_DebugPrintf("CUT LIST:\n");
	ajbsp_DebugPrintf("PARTITION: (%1.1f,%1.1f) += (%1.1f,%1.1f)\n",
			part->psx, part->psy, part->pdx, part->pdy);

	for (cur=cut_list ; cur ; cur=cur->next)
	{
		ajbsp_DebugPrintf("  Vertex %8X (%1.1f,%1.1f)  Along %1.2f  [%d/%d]  %s\n",
...
# if DEBUG_CUTLIST
		ajbsp_DebugPrintf("Merging cut (%1.0f,%1.0f) [%d/%d] with %p (%1.0f,%1.0f) [%d/%d]\n",
...
# if DEBUG_CUTLIST
		ajbsp_DebugPrintf("---> merged (%1.0f,%1.0f) [%d/%d] %s\n",
				cur->vertex->x, cur->vertex->y,
				cur->before ? cur->before->index : -1,
				cur->after ? cur->after->index : -1,
				cur->self_ref ? "SELFREF" : "");
...
#   if DEBUG_CUTLIST
		ajbsp_DebugPrintf("AddMiniseg: %p RIGHT  sector %d  (%1.1f,%1.1f) -> (%1.1f,%1.1f)\n",
				seg, seg->sector ? seg->sector->index : -1,
				seg->start->x, seg->start->y, seg->end->x, seg->end->y);

		ajbsp_DebugPrintf("AddMiniseg: %p LEFT   sector %d  (%1.1f,%1.1f) -> (%1.1f,%1.1f)\n",
...
void TestSuper(superblock_t *block)
{
	...
	if (real_num != block->real_num || mini_num != block->mini_num)
		ajbsp_BugError("TestSuper FAILED: block=%p %d/%d != %d/%d\n",
...
# if DEBUG_SUBSEC
	ajbsp_DebugPrintf("Subsec: Clockwising %d\n", sub->index);
# endif
	...
	if (i != total)
		ajbsp_BugError("ClockwiseOrder miscounted.\n");
...
# if DEBUG_SORTER
	ajbsp_DebugPrintf("Sorted SEGS around (%1.1f,%1.1f)\n", sub->mid_x, sub->mid_y);
	...
	for (seg=sub->seg_list ; seg ; seg=seg->next)
	{
		...
		ajbsp_DebugPrintf("  Seg %p: Angle %1.6f  (%1.1f,%1.1f) -> (%1.1f,%1.1f)\n",
...
#   if DEBUG_SUBSEC
		for (seg=sub->seg_list ; seg ; seg=seg->next)
		{
			ajbsp_DebugPrintf("  SEG %p  (%1.1f,%1.1f) --> (%1.1f,%1.1f)\n", seg,
					seg->start->x, seg->start->y, seg->end->x, seg->end->y);
		}
#   endif
	}
...
static void SanityCheckHasRealSeg(subsec_t *sub)
{
	...
	ajbsp_BugError("Subsector #%d near (%1.1f,%1.1f) has no real seg!\n",
...
# if DEBUG_SUBSEC
	ajbsp_DebugPrintf("Subsec: Renumbering %d\n", sub->index);
# endif
...
#   if DEBUG_SUBSEC
		ajbsp_DebugPrintf("Subsec:   %d: Seg %p  Index %d\n", sub->seg_count,
				seg, seg->index);
#   endif
	}
}
...
static void CreateSubsecWorker(subsec_t *sub, superblock_t *block)
{
	...
	for (num=0 ; num < 2 ; num++)
	{
		...
		if (A)
		{
			...
			if (A->real_num + A->mini_num > 0)
				ajbsp_BugError("CreateSubsec: child %d not empty!\n", num);
...
# if DEBUG_SUBSEC
	ajbsp_DebugPrintf("Subsec: Creating %d\n", sub->index);
# endif
...
static void DebugShowSegs(superblock_t *seg_list)
{
	...
	{
		ajbsp_DebugPrintf("Build:   %sSEG %p  sector=%d  (%1.1f,%1.1f) -> (%1.1f,%1.1f)\n",
...
# if DEBUG_BUILDER
	ajbsp_DebugPrintf("Build: BEGUN @ %d\n", depth);
...
#   if DEBUG_BUILDER
		ajbsp_DebugPrintf("Build: CONVEX\n");
#   endif
...
# if DEBUG_BUILDER
	ajbsp_DebugPrintf("Build: PARTITION %p (%1.0f,%1.0f) -> (%1.0f,%1.0f)\n",
...
# endif
	...
	if (rights->real_num + rights->mini_num == 0)
		ajbsp_BugError("Separated seg-list has no RIGHT side\n");

	if (lefts->real_num + lefts->mini_num == 0)
		ajbsp_BugError("Separated seg-list has no LEFT side\n");
...
# if DEBUG_BUILDER
	ajbsp_DebugPrintf("Build: Going LEFT\n");
# endif
...
# if DEBUG_BUILDER
	ajbsp_DebugPrintf("Build: Going RIGHT\n");
# endif
...
# if DEBUG_SUBSEC
	ajbsp_DebugPrintf("Subsec: Normalising %d\n", sub->index);
# endif
	...
	while (sub->seg_list)
	{
		...
		{
#     if DEBUG_SUBSEC
			ajbsp_DebugPrintf("Subsec: Removing miniseg %p\n", seg);
#     endif
	...
	if (new_head == NULL)
		ajbsp_BugError("Subsector %d normalised to being EMPTY\n", sub->index);
...
# if DEBUG_SUBSEC
	ajbsp_DebugPrintf("Subsec: Rounding off %d\n", sub->index);
# endif
...
# if DEBUG_SUBSEC
	ajbsp_DebugPrintf("Subsec: degen=%d real=%d\n", degen_total, real_total);
# endif
	...
	if (real_total == 0)
	{
		if (last_real_degen == NULL)
			ajbsp_BugError("Subsector %d rounded off with NO real segs\n",
					sub->index);

#   if DEBUG_SUBSEC
		ajbsp_DebugPrintf("Degenerate before: (%1.2f,%1.2f) -> (%1.2f,%1.2f)\n",
...
#   if DEBUG_SUBSEC
		ajbsp_DebugPrintf("Degenerate after:  (%d,%d) -> (%d,%d)\n",
...
#   endif
	...
	while (sub->seg_list)
	{
		...
		{
#     if DEBUG_SUBSEC
			ajbsp_DebugPrintf("Subsec: Removing degenerate %p\n", seg);
#     endif
	...
	if (new_head == NULL)
		ajbsp_BugError("Subsector %d rounded off to being EMPTY\n", sub->index);
