...
void *UtilCalloc(int size)
{
	...
	if (!ret)
		ajbsp_FatalError("Out of memory (cannot allocate %d bytes)\n", size);
...
void *UtilRealloc(void *old, int size)
{
	...
	if (!ret)
		ajbsp_FatalError("Out of memory (cannot reallocate %d bytes)\n", size);
...
{
	if (data == NULL)
		ajbsp_BugError("Trying to free a NULL pointer\n");
...
# if DEBUG_POLYOBJ
	ajbsp_DebugPrintf("  Marking SECTOR %d\n", sector->index);
# endif
...
static void MarkPolyobjPoint(double x, double y)
{
	...
	for (i = 0 ; i < num_linedefs ; i++)
	{
		...
		{
#     if DEBUG_POLYOBJ
			ajbsp_DebugPrintf("  Touching line was %d\n", L->index);
#     endif
...
# if DEBUG_POLYOBJ
	ajbsp_DebugPrintf("  Closest line was %d Y=%1.0f..%1.0f (dist=%1.1f)\n",
...
# if DEBUG_POLYOBJ
	...
	{
		ajbsp_DebugPrintf("  Polyobj FAILURE: directly on the line (%d)\n",
				best_match->index);
	}
# endif
...
# if DEBUG_POLYOBJ
	ajbsp_DebugPrintf("  Sector %d contains the polyobj.\n",
...
# if DEBUG_POLYOBJ
	ajbsp_DebugPrintf("Using %s style polyobj things\n",
...
#   if DEBUG_POLYOBJ
		ajbsp_DebugPrintf("Thing %d at (%1.0f,%1.0f) is a polyobj spawner.\n", i, x, y);
#   endif
...
# if DEBUG_WALLTIPS
	for (i=0 ; i < num_vertices ; i++)
	{
		...
		ajbsp_DebugPrintf("WallTips for vertex %d:\n", i);

		for (wall_tip_t *tip = V->tip_set ; tip ; tip = tip->next)
		{
			ajbsp_DebugPrintf("  Angle=%1.1f left=%d right=%d\n", tip->angle,
...
vertex_t *NewVertexDegenerate(vertex_t *start, vertex_t *end)
{
	...
	if (dlen == 0)
		ajbsp_BugError("NewVertexDegenerate: bad delta!\n");
...
sector_t * VertexCheckOpen(vertex_t *vert, double dx, double dy)
{
	...
	ajbsp_BugError("Vertex %d has no tips!\n", vert->index);
	return NULL;
}
