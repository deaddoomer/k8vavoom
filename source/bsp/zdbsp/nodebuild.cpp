...
void FNodeBuilder::CreateSubsectorsForReal ()
{
  ...
  for (i = 0; i < SubsectorSets.Size(); ++i)
  {
    ...
    for (unsigned int f = sub.firstline; f < SegList.Size(); ++f)
    {
      D(printf ("  Seg %5d%c%d(%5d,%5d)-%d(%5d,%5d)  [%08x,%08x]-[%08x,%08x]\n", SegList[f].SegPtr - &Segs[0],
        SegList[f].SegPtr->linedef == -1 ? '+' : ' ',
        SegList[f].SegPtr->v1,
        Vertices[SegList[f].SegPtr->v1].x>>16,
        Vertices[SegList[f].SegPtr->v1].y>>16,
        SegList[f].SegPtr->v2,
        Vertices[SegList[f].SegPtr->v2].x>>16,
        Vertices[SegList[f].SegPtr->v2].y>>16,
        Vertices[SegList[f].SegPtr->v1].x, Vertices[SegList[f].SegPtr->v1].y,
        Vertices[SegList[f].SegPtr->v2].x, Vertices[SegList[f].SegPtr->v2].y));
      SegList[f].SegNum = DWORD(SegList[f].SegPtr - &Segs[0]);
    }
    Subsectors.Push (sub);
  }
}
