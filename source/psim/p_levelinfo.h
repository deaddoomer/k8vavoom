...
public:
  ...
  void eventPolyRotateMobj (VEntity *A, polyobj_t *po, float dangle) { static VMethodProxy method("PolyRotateMobj"); vobjPutParamSelf(A, po, dangle); VMT_RET_VOID(method); }
  bool eventPolyMoveMobjBy (VEntity *A, polyobj_t *po, float dx, float dy) { static VMethodProxy method("PolyMoveMobjBy"); vobjPutParamSelf(A, po, dx, dy); VMT_RET_BOOL(method); }
